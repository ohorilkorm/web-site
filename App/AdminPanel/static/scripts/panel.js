...
function getData (data, l1, l2) {
	if (!isSort)
	{
		...
		$.getJSON("http://yaroslxw.beget.tech/api/" + data, function(result){
	    	totalNum = result.length;
	    	$.getJSON("http://yaroslxw.beget.tech/api/" + data + "?limit1="+l1+"&limit2="+l2, function(result){
...
function updateRecord(formId, elemId, table){
	...
	$.ajax({
	  method: "PUT",
	  url: "http://yaroslxw.beget.tech/api/" + table + "/" + elemId + "?" + str,
...
function deleteRecord(table){
	$.ajax({
	  method: "DELETE",
	  url: "http://yaroslxw.beget.tech/api/" + table + "/" + elemId,
	  success: function(dat) {
	  		 getData(table,10,0);
		}
	});
...
function createRecord(table){
	...
	$.ajax({
	  method: "POST",
	  url: "http://yaroslxw.beget.tech/api/" + table + "?" + str,
	  success: function(dat) {
	  		getData(table,10,0);
		}
	});	
...
function changePassword()
{
	...
	if (newpas1==newpas2)
	{
		$.ajax({
		  method: "PUT",
		  url: "http://yaroslxw.beget.tech/admin/changepassword?oldPassowrd="+oldpas+"&newPassword="+newpas1,
		  success: function(dat) {
				if(dat.Status=='Error')
				{
...
function buildLink(l1, l2)
{
	...
	else
	{
		...
		$.getJSON("http://yaroslxw.beget.tech/api/" + link.toLowerCase(), function(result){
	    	totalNum = result.length;
	    	$.getJSON("http://yaroslxw.beget.tech/api/" + link.toLowerCase() + "?limit1="+l1+"&limit2="+l2, function(result){
	    		makeTable(result, l1, l2);
	    	});
	    });
	}
